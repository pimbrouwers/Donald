...
module Sql =
    ...
    type DbParam = { Name : string; Value : obj }
    ...
    let beginTran (conn : IDbConnection) = 
        if conn.State <> ConnectionState.Open
           && conn.State <> ConnectionState.Closed then conn.Open()
    ...
    let commitTran (tran : IDbTransaction) =
        try
           tran.Commit() 
        with
           | :? InvalidOperationException as ex -> reraise() // tran is commmited or rolled back already
           | _ -> rollbackTran tran
    ...
    type IDbConnection with
        ...
        member this.Scalar sql param convert =
            use tran = beginTran this
            let v = scalar sql convert param tran
            commitTran tran
            v
